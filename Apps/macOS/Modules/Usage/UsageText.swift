...
/// limit refills, what went wrong (one clear sentence each, PLAN §4) and the status Settings shows per provider.
enum UsageText {
    // MARK: Windows
    ...
    static func refillsIn(_ window: UsageWindow, now: Date = .now) -> String {
        guard let resetsAt = window.resetsAt else { return String(localized: "starts with your next message") }
    ...
    static func status(for entry: UsageStore.Entry, now: Date = .now) -> String {
        ...
        let source = entry.isExternal ? String(localized: "Connected through OpenUsage") : String(localized: "Connected")
        if let plan = usage.plan, !plan.isEmpty { return "\(source) · \(plan)" }
        return source
    }
}
