...
struct AssistantWebTests {
    ...
    @Test func eachRouteGetsShortInstructions() {
        ...
        #expect(context.contains("shelf, calendar, nowPlaying, clipboard, clipboardHistory, usage or agents"))
        ...
        #expect(AssistantTools.all(for: .context, shelfItems: { [] }, report: { _ in }).map(\.name)
            == ["shelf", "calendar", "nowPlaying", "clipboard", "clipboardHistory", "usage", "agents", "timer", "note"])
    }
