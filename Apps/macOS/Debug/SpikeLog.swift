...
private struct SpikeToolsView: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Text("Herramientas").foregroundStyle(.secondary)
            Button("Probar Quick Look") {
                if let url = Self.makeSampleFile(named: "Prueba Quick Look.txt") {
                    QuickLookPresenter.show([url])
                }
            }
            Label("Archivo de prueba", systemImage: "doc")
                .padding(.horizontal, 10)
                .padding(.vertical, 5)
                .background(.quaternary, in: .capsule)
                .help("Arrástralo fuera para probar el drag out (se crea un archivo nuevo en el Inbox)")
    ...
    private func ensureSample() -> [ShelfItem] {
        ...
        guard let url = Self.makeSampleFile(named: "Archivo de prueba.txt") else { return [] }
    ...
    private static func makeSampleFile(named name: String) -> URL? {
        do {
            ...
            try Data("Archivo de prueba de Altillo — \(Date.now.formatted())\n".utf8).write(to: url)
            return url
        } catch {
            SpikeLog.shared.record(SpikeLog.Category.ingest, "FALLO creando el archivo de prueba: \(error.localizedDescription)")
            return nil
        }
    }
}
